...
## 🎬 4분 시연 — 마커별 멘트 (총 230초 + 여유 10초)

> **budget 배분**: 마커 0 도입 30s, 마커 4·7·10 각 22s, 마커 8 (β slider 핵심) 28s,
> 나머지 평균 18~20s. 총 230s. 마커 시각 라벨(0:00/0:15/…)은 narrative time일 뿐,
> mason 의 실제 머무는 시간은 자율.

### Marker 0 (도입, ~30s) — 화면 구성 + 정상 상태
```
"정상 가동 상태입니다. 시연 시작 전에 화면 구성 잠깐 보시면 —

왼쪽 사이드바에 PRISM 제어판이 다 모여 있어요.
첫째, σ_max 0.40 robust — Wright 1991 학술 기준 통과한 인과 모델.
둘째, DuckDB lineage Bronze → Silver → Gold, 실시간 sensor 적재.
셋째, Bedrock 상태와 α/β/γ slider — 평가자께서 직접 조작 가능합니다.

가운데는 6-노드 인과 DAG, 위쪽엔 KPI 4개 — OEE, RCA, Defect, 비용.
지금은 모두 정상 범위입니다. 이제 흐름 시작하겠습니다."
```
