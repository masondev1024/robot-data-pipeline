...
# The provider is account-shared, so existing accounts must use the data source
# and must not let this project destroy it during teardown. Set
# manage_github_oidc_provider=true only in an account where it is absent.
